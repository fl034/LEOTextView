...
open class LEOTextView: UITextView {
    ...
    open var defaultListIndent: CGFloat = 1.0
            ...
            if attributeName == NSAttributedStringKey.font.rawValue {
                ...
                if listTypeRawValue != nil {
                    ...
                    if defaultParagraphStyle != nil {
                        ...
                        paragraphStyle = LEOTextView(frame: .zero).mutableParagraphForList(withBulletPointWidth: listPrefixWidth)
                    }
    ...
    func mutableParargraphWithDefaultSetting() -> NSMutableParagraphStyle {
        ...
        if let defaultParagraphStyle = defaultAttributesForLoad[.paragraphStyle] as? NSParagraphStyle {
            paragraphStyle = defaultParagraphStyle.mutableCopy() as? NSMutableParagraphStyle
        } else {
    ...
    func mutableParagraphForList(withBulletPointWidth bulletPointWidth: CGFloat) -> NSMutableParagraphStyle {
        let paragraphStyle = mutableParargraphWithDefaultSetting()
        paragraphStyle.firstLineHeadIndent = defaultListIndent
        paragraphStyle.headIndent = bulletPointWidth + defaultListIndent
        return paragraphStyle
    }
    ...
    open override func paste(_ sender: Any?) {
        ...
        pasteText.enumerateLines { [unowned self] (line, stop) in
            ...
            if LEOTextUtil.markdownOrderedListRegularExpression.matches(in: line, options: .reportProgress, range: NSMakeRange(0, lineLength)).count > 0 ||
                ...
                let paragraphStyle = self.mutableParagraphForList(withBulletPointWidth: listPrefixString.size(withAttributes: [NSAttributedStringKey.font: self.normalFont]).width)
